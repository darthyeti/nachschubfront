...
// section 9 ("Wellenaufbau"). Run with `npm run waves`.
//
...
/** First wave an enemy type can appear in; the cycle introduces them in order. */
const UNLOCK = {
  swarmer: 1,
  warrior: 1,
  breaker: 2,
...
const share = (wave, type, fraction) => {
  ...
  const base = waveCount(wave) * (type === 'swarmer' ? SWARMER_FACTOR : 1);
...
const header = `// The complete wave list (GDD section 9, "Wellenaufbau").
//
