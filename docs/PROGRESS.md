...
- **`npm run test:battle` scheitert schon vor M4d**, auf 6913202 genauso wie danach: Der Durchlauf verliert Welle 1 mit 20 Durchbrüchen und null Abschüssen. Die Prüfung setzt fünf Landezonen neben die Route und nimmt dann den stärksten Vorschlag des Auswahldialogs — der verschmilzt vier Kapseln zu einer einzigen Stellung, und eine Stellung hält Welle 1 seit M4b nicht mehr auf. `docs/PROGRESS.md` beschreibt diese Prüfung als Regressionsprüfung ("läuft eine ganze Partie fehlerfrei durch") und sagt, die Bastion werde dabei mit dem Debug-Hebel abgestützt; `tests/battle.mjs` drückt "Unverwundbar" aber nirgends. Der kleinste Eingriff wäre genau das: den Hebel einschalten, damit die Prüfung wieder das prüft, wofür sie da ist. Das ist keine Balancing-Änderung, aber auch nicht Teil von M4d — steht zur Entscheidung.
