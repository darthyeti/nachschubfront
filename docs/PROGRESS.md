...
## Offen
- **Zwei Fragen für M5, noch nicht entschieden.** Sie kamen beim ersten M5-Plan auf (23.09.2026) und blieben liegen, weil Update 4 dazwischenkam:
  - *Import: ersetzen oder zusammenführen?* Vorschlag: ersetzen, mit Bestätigung. Zusammenführen klingt freundlicher, zählt aber die Statistiksummen doppelt, wenn dieselbe Datei zweimal importiert wird, und das merkt man nicht.
  - *Einstellungen mit ins Profil?* Vorschlag: ja — ein Dokument, ein Export, eine Migration vom heutigen `prefs`-Schlüssel. Nachteil: Wer ein fremdes Profil importiert, bekommt auch dessen Lautstärken.
  Der GDD gibt zu beidem nichts vor. Der übrige M5-Plan (Profil mit Versionierung und Migration, Bestwerte je Seed und je Regelversion, Statistiken, Export/Import mit Einfügefeld als Rückfallebene, Bestenliste im Menü, Service Worker mit erzeugter und testgeprüfter Precache-Liste, Installationshinweis, `docs/SPEICHER.md`) steht im Verlauf und lässt sich aus `docs/meilensteine/M5-speichern-und-pwa.md` neu aufbauen.
